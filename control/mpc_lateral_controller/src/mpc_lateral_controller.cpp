...
MpcLateralController::MpcLateralController(rclcpp::Node::SharedPtr node_ptr)
: node_weak_ptr_{node_ptr}
{
  using std::placeholders::_1;

  m_mpc.m_ctrl_period = node_ptr->get_parameter("ctrl_period").as_double();
  m_enable_path_smoothing = node_ptr->declare_parameter<bool>("enable_path_smoothing");
  m_path_filter_moving_ave_num = node_ptr->declare_parameter<int>("path_filter_moving_ave_num");
  m_curvature_smoothing_num_traj = node_ptr->declare_parameter<int>("curvature_smoothing_num_traj");
  m_curvature_smoothing_num_ref_steer =
    node_ptr->declare_parameter<int>("curvature_smoothing_num_ref_steer");
  m_traj_resample_dist = node_ptr->declare_parameter<double>("traj_resample_dist");
  m_mpc.m_admissible_position_error =
    node_ptr->declare_parameter<double>("admissible_position_error");
  m_mpc.m_admissible_yaw_error_rad =
    node_ptr->declare_parameter<double>("admissible_yaw_error_rad");
  m_mpc.m_use_steer_prediction = node_ptr->declare_parameter<bool>("use_steer_prediction");
  m_mpc.m_param.steer_tau = node_ptr->declare_parameter<double>("vehicle_model_steer_tau");
  m_extend_trajectory_for_end_yaw_control =
    node_ptr->declare_parameter<bool>("extend_trajectory_for_end_yaw_control");

  /* stop state parameters */
  m_stop_state_entry_ego_speed = node_ptr->declare_parameter<double>("stop_state_entry_ego_speed");
  m_stop_state_entry_target_speed =
    node_ptr->declare_parameter<double>("stop_state_entry_target_speed");
  m_converged_steer_rad = node_ptr->declare_parameter<double>("converged_steer_rad");
  m_keep_steer_control_until_converged =
    node_ptr->declare_parameter<bool>("keep_steer_control_until_converged");
  m_new_traj_duration_time = node_ptr->declare_parameter<double>("new_traj_duration_time");  // [s]
  m_new_traj_end_dist = node_ptr->declare_parameter<double>("new_traj_end_dist");            // [m]

  /* mpc parameters */
  const auto vehicle_info = vehicle_info_util::VehicleInfoUtil(*node_ptr).getVehicleInfo();
  const double wheelbase = vehicle_info.wheel_base_m;
  const double steer_rate_lim_dps = node_ptr->declare_parameter<double>("steer_rate_lim_dps");
  ...
  const std::string vehicle_model_type =
    node_ptr->declare_parameter<std::string>("vehicle_model_type");
  ...
  if (vehicle_model_type == "kinematics") {
    ...
    const double mass_fl = node_ptr->declare_parameter<double>("vehicle.mass_fl");
    const double mass_fr = node_ptr->declare_parameter<double>("vehicle.mass_fr");
    const double mass_rl = node_ptr->declare_parameter<double>("vehicle.mass_rl");
    const double mass_rr = node_ptr->declare_parameter<double>("vehicle.mass_rr");
    const double cf = node_ptr->declare_parameter<double>("vehicle.cf");
    const double cr = node_ptr->declare_parameter<double>("vehicle.cr");
    ...
    RCLCPP_ERROR(node_ptr->get_logger(), "vehicle_model_type is undefined");
  }

  /* QP solver setup */
  const std::string qp_solver_type = node_ptr->declare_parameter<std::string>("qp_solver_type");
  ...
  if (qp_solver_type == "unconstraint_fast") {
    ...
    qpsolver_ptr = std::make_shared<QPSolverOSQP>(node_ptr->get_logger());
  } else {
    RCLCPP_ERROR(node_ptr->get_logger(), "qp_solver_type is undefined");
  }

  /* delay compensation */
  {
    const double delay_tmp = node_ptr->declare_parameter<double>("input_delay");
  ...
  /* steering offset compensation */
  {
    ...
    enable_auto_steering_offset_removal_ =
      node_ptr->declare_parameter<bool>(ns + "enable_auto_steering_offset_removal");
    const auto vel_thres = node_ptr->declare_parameter<double>(ns + "update_vel_threshold");
    const auto steer_thres = node_ptr->declare_parameter<double>(ns + "update_steer_threshold");
    const auto limit = node_ptr->declare_parameter<double>(ns + "steering_offset_limit");
    const auto num = node_ptr->declare_parameter<int>(ns + "average_num");
    steering_offset_ =
  ...
  /* initialize lowpass filter */
  {
    const double steering_lpf_cutoff_hz =
      node_ptr->declare_parameter<double>("steering_lpf_cutoff_hz");
    const double error_deriv_lpf_cutoff_hz =
      node_ptr->declare_parameter<double>("error_deriv_lpf_cutoff_hz");
    m_mpc.initializeLowPassFilters(steering_lpf_cutoff_hz, error_deriv_lpf_cutoff_hz);
  }

  // ego nearest index search
  m_ego_nearest_dist_threshold =
    node_ptr->has_parameter("ego_nearest_dist_threshold")
      ? node_ptr->get_parameter("ego_nearest_dist_threshold").as_double()
      : node_ptr->declare_parameter<double>("ego_nearest_dist_threshold");  // [m]
  m_ego_nearest_yaw_threshold =
    node_ptr->has_parameter("ego_nearest_yaw_threshold")
      ? node_ptr->get_parameter("ego_nearest_yaw_threshold").as_double()
      : node_ptr->declare_parameter<double>("ego_nearest_yaw_threshold");  // [rad]
  m_mpc.ego_nearest_dist_threshold = m_ego_nearest_dist_threshold;
  m_mpc.ego_nearest_yaw_threshold = m_ego_nearest_yaw_threshold;

  m_pub_predicted_traj = node_ptr->create_publisher<autoware_auto_planning_msgs::msg::Trajectory>(
    "~/output/predicted_trajectory", 1);
  m_pub_debug_values = node_ptr->create_publisher<tier4_debug_msgs::msg::Float32MultiArrayStamped>(
    "~/output/lateral_diagnostic", 1);
  m_pub_steer_offset = node_ptr->create_publisher<tier4_debug_msgs::msg::Float32Stamped>(
  ...
  m_set_param_res = node_ptr->add_on_set_parameters_callback(
  ...
  m_mpc.setLogger(node_ptr->get_logger());
  m_mpc.setClock(node_ptr->get_clock());
}
  ...
  trajectory_follower::InputData const & input_data)
{
  ...
  if (!is_mpc_solved) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_WARN_SKIPFIRST_THROTTLE(
        node_ptr->get_logger(), *node_ptr->get_clock(), 5000 /*ms*/,
  ...
  const autoware_auto_control_msgs::msg::AckermannLateralCommand & cmd) const
{
  ...
  if (!m_has_received_first_trajectory || isTrajectoryShapeChanged()) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_DEBUG(node_ptr->get_logger(), "trajectory shaped is changed");
    }
    return false;
  }
...
bool MpcLateralController::isReady(const trajectory_follower::InputData & input_data)
{
  ...
  if (!m_mpc.hasVehicleModel()) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_INFO_THROTTLE(
        node_ptr->get_logger(), *node_ptr->get_clock(), 5000, "MPC does not have a vehicle model");
    }
    return false;
  }
  if (!m_mpc.hasQPSolver()) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_INFO_THROTTLE(
        node_ptr->get_logger(), *node_ptr->get_clock(), 5000, "MPC does not have a QP solver");
    }
    return false;
  }
  if (m_mpc.m_ref_traj.size() == 0) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_INFO_THROTTLE(
        node_ptr->get_logger(), *node_ptr->get_clock(), 5000, "trajectory size is zero.");
    }
    return false;
  }
...
void MpcLateralController::setTrajectory(const autoware_auto_planning_msgs::msg::Trajectory & msg)
{
  ...
  if (msg.points.size() < 3) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_DEBUG(node_ptr->get_logger(), "received path size is < 3, not enough.");
    }
    return;
  }
  ...
  if (!isValidTrajectory(msg)) {
    if (auto node_ptr = node_weak_ptr_.lock()) {
      RCLCPP_ERROR(node_ptr->get_logger(), "Trajectory is invalid!! stop computing.");
    }
    return;
  }
...
{
  if (auto node_ptr = node_weak_ptr_.lock()) {
    predicted_traj.header.stamp = node_ptr->now();
...
{
  if (auto node_ptr = node_weak_ptr_.lock()) {
    m_mpc.m_param.prediction_horizon = node_ptr->declare_parameter<int>("mpc_prediction_horizon");
    m_mpc.m_param.prediction_dt = node_ptr->declare_parameter<double>("mpc_prediction_dt");
    m_mpc.m_param.weight_lat_error = node_ptr->declare_parameter<double>("mpc_weight_lat_error");
    m_mpc.m_param.weight_heading_error =
      node_ptr->declare_parameter<double>("mpc_weight_heading_error");
    m_mpc.m_param.weight_heading_error_squared_vel =
      node_ptr->declare_parameter<double>("mpc_weight_heading_error_squared_vel");
    m_mpc.m_param.weight_steering_input =
      node_ptr->declare_parameter<double>("mpc_weight_steering_input");
    m_mpc.m_param.weight_steering_input_squared_vel =
      node_ptr->declare_parameter<double>("mpc_weight_steering_input_squared_vel");
    m_mpc.m_param.weight_lat_jerk = node_ptr->declare_parameter<double>("mpc_weight_lat_jerk");
    m_mpc.m_param.weight_steer_rate = node_ptr->declare_parameter<double>("mpc_weight_steer_rate");
    m_mpc.m_param.weight_steer_acc = node_ptr->declare_parameter<double>("mpc_weight_steer_acc");
    m_mpc.m_param.low_curvature_weight_lat_error =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_lat_error");
    m_mpc.m_param.low_curvature_weight_heading_error =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_heading_error");
    m_mpc.m_param.low_curvature_weight_heading_error_squared_vel =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_heading_error_squared_vel");
    m_mpc.m_param.low_curvature_weight_steering_input =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_steering_input");
    m_mpc.m_param.low_curvature_weight_steering_input_squared_vel =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_steering_input_squared_vel");
    m_mpc.m_param.low_curvature_weight_lat_jerk =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_lat_jerk");
    m_mpc.m_param.low_curvature_weight_steer_rate =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_steer_rate");
    m_mpc.m_param.low_curvature_weight_steer_acc =
      node_ptr->declare_parameter<double>("mpc_low_curvature_weight_steer_acc");
    m_mpc.m_param.low_curvature_thresh_curvature =
      node_ptr->declare_parameter<double>("mpc_low_curvature_thresh_curvature");
    m_mpc.m_param.weight_terminal_lat_error =
      node_ptr->declare_parameter<double>("mpc_weight_terminal_lat_error");
    m_mpc.m_param.weight_terminal_heading_error =
      node_ptr->declare_parameter<double>("mpc_weight_terminal_heading_error");
    m_mpc.m_param.zero_ff_steer_deg = node_ptr->declare_parameter<double>("mpc_zero_ff_steer_deg");
    m_mpc.m_param.acceleration_limit =
      node_ptr->declare_parameter<double>("mpc_acceleration_limit");
    m_mpc.m_param.velocity_time_constant =
      node_ptr->declare_parameter<double>("mpc_velocity_time_constant");
    m_mpc.m_param.min_prediction_length =
      node_ptr->declare_parameter<double>("mpc_min_prediction_length");
  }
}
